...
class LoginView: UIViewController, LoginViewType {
    ...
    private let loginContainer: UIView = UIView.containerView(background: ViewProperties.loginContainerColor, radius: Layout.loginContainerRadius).addingShadow(4, dy: 4, color: ViewProperties.loginContainerShadow, radius: 10, opacity: 0.8)
    ...
    private func setViews() {
        ...
        loginContainer.add(views: emailField, emailSeparatingBar, passwordField, passwordSeparatingBar, loginButton, alertScreen)
        constrainSubViewSafely(subView: signUpButton, top: Layout.signUpButtonEdgeOffset.top, right: Layout.signUpButtonEdgeOffset.right)
        constrainSubViewSafely(subView: backButton, top: Layout.backButtonEdgeOffset, left: Layout.backButtonEdgeOffset)
        loginTitle.setTopTo(con: self.view.top(), by: Layout.loginTitleEdgeOffset.top)
        constrainSubViewSafely(subView: loginTitle, top: Layout.loginTitleEdgeOffset.top)
